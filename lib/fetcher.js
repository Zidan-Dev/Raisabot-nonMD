...
exports.getBuffer = getBuffer = async (url) => {
    ...
    const anu = fs.readFileSync('./database/media/emror.jpg')
...
exports.kyun = (seconds) =>{
    ...
    return `${pad(hours)} 𝐽𝑎𝑚 ${pad(minutes)} 𝑀𝑒𝑛𝑖𝑡 ${pad(seconds)} 𝐷𝑒𝑡𝑖𝑘`
}
exports.createExif = (pack, auth) =>{
    const code = [0x00,0x00,0x16,0x00,0x00,0x00]
    const exif = {"sticker-pack-id": "com.thunderx7.tech", "sticker-pack-name": pack, "sticker-pack-publisher": auth, "android-app-store-link": "https://play.google.com/store/apps/details?id=com.termux", "ios-app-store-link": "https://itunes.apple.com/app/sticker-maker-studio/id1443326857"}
...
exports.modStick = (media, thunderx7, thunder, from) => {
    out = getRandom('.webp')
    try {
        console.log(media)
        spawn('webpmux', ['-set','exif', './database/stik/data.exif', media, '-o', out])
        .on('exit', () => {
            thunderx7.sendMessage(from, fs.readFileSync(out), 'stickerMessage', {quoted: thunder})
            fs.unlinkSync(out)
            fs.unlinkSync(media)
        })
    } catch (e) {
        console.log(e)
        thunderx7.sendMessage(from, 'Terjadi keslahan', 'conversation', { quoted: thunder })
        fs.unlinkSync(media)
    }
}
